...
class NewsScreen(Screen):
    ...
    def on_enter(self):
        self.buttons = [
            ...
            Button((166, 24, 130, 56), "Actualizar", self._load_active_feed, bg=theme.SURFACE),
            Button((308, 24, 120, 56), "Detener", voice_service.stop, bg=theme.SURFACE),
        ]
    ...
    def _toggle_item(self, index):
        ...
        if not item["expanded"]:
            ...
            spoken_text = _truncate(f"{item['title']}. {item['content']}".strip(), MAX_SPOKEN_CHARS)
    ...
    def _draw_card(self, surface, item, index, x, y, width):
        ...
        chevron_center = (x + width - PADDING - 16, content_y + 16)
        pygame.draw.circle(surface, theme.INDIGO if item["expanded"] else theme.GRAY_NEUTRAL, chevron_center, 16)
        chevron = "-" if item["expanded"] else "+"
        chevron_surf = theme.FONT_BODY.render(chevron, True, (255, 255, 255))
        surface.blit(chevron_surf, chevron_surf.get_rect(center=chevron_center))
        ...
        if item["expanded"]:
            if item["image_loading"]:
                ...
                img_rect = item["image_surface"].get_rect(topleft=(x + PADDING, line_y))
                pygame.draw.rect(
                    surface, theme.BG, img_rect.inflate(8, 8), border_radius=12
                )
                surface.blit(item["image_surface"], img_rect)
                line_y += item["image_surface"].get_height() + 16
